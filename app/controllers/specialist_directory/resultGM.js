    ...
    Alloy.Globals.API.callByPost({url: "getSpecialistV2", new:true, domain: "FREEJINI_DOMAIN",  params: {name: name, state: state, specialty: specialty, hospital: hospital, page: page, limit: 10}}, function(responseText){
        ...
        if(obj.data.length != 0){
            ...
            bNext.addEventListener("click", function(){
                ...
                init();
                if(OS_IOS){
                    Alloy.Globals.nav.navigationWindow("specialist_directory/resultGM", "", "", {data: obj, page: page, name: name, state: state, specialty: specialty, hospital: hospital, gm:gm});
                } else{
                    var win = Alloy.createController("specialist_directory/resultGM", {data: obj, page: page, name: name, state: state, specialty: specialty, hospital: hospital, gm:gm}).getView();
                    win.open();
                }
            });
            ...
            if(gm < 6){
                specialty = GMList[gm];
                Alloy.Globals.API.callByPost({url: "getSpecialistV2", new:true, domain: "FREEJINI_DOMAIN",  params: {name: name, state: state, specialty: specialty, hospital: hospital, page: newPage, limit: 10}}, function(responseText){
                    ...
                    if(obj.data.length != 0){
                        ...
                        bNext.addEventListener("click", function(){
                            ...
                            init();
                            if(OS_IOS){
                                Alloy.Globals.nav.navigationWindow("specialist_directory/resultGM", "", "", {data: obj, page: newPage, name: name, state: state, specialty: specialty, hospital: hospital, gm:gm});
                            } else{
                                var win = Alloy.createController("specialist_directory/resultGM", {data: obj, page: newPage, name: name, state: state, specialty: specialty, hospital: hospital, gm:gm}).getView();
                                win.open();
                            }
                        });
                    }
