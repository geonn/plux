...
exports.MediaPicker.prototype.show = function (cb, max, type, message) {
  ...
  if (_env.iOS) win_assetGroups.setLeftNavButton(cancel);else
  win_assetGroups.add(cancel);
  ...
  var gotAssets = function (e) {
    ...
    win_assets.addEventListener('close', function () {
      ...
      for (var i = 0; i < assets.length; i++) {
        for (var j = 0; j < assets[i].length; j++) {
          ...
          if (!iv) continue;
          iv.image = null;
          if (_env.iOS) rows[iv.row].remove(iv);else
          grid.remove(iv);
          iv = null;
        }
      }
    ...
    if (_env.iOS) win_assets.setRightNavButton(done);else
    win_assets.add(done);
  ...
  var loadImages = function (row, lzy) {
    ...
    for (var i = 0; i < toAdd.length; i++) {
      if (toAdd[i].iv) {
        ...
        if (_env.iOS) {rows[row].add(iv);} else
        grid.add(iv);
      }
  ...
  function createIV(row, i) {
    ...
    iv.addEventListener('longpress', function (e) {
      ...
      if (type == 'videos') {
        ...
        var iv = e.source;
        if (_env.iOS) activeMovie.media = assets[iv.row][iv.i].url;else
        activeMovie.url = 'file://' + assets[iv.row][iv.i].url;
