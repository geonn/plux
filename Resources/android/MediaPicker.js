...
var warned = false;
var _env = {};
_env.iOS = 'android' == 'iphone' || 'android' == 'ipad';
...
var _config = {
	size: 'android' == 'ipad' ? 148 : 'android' == 'iphone' ? 75 : 96,
	dpi_warning: 1024
};
var _lang = {
	photos: 'Images',
...
exports.MediaPicker.prototype.show = function (cb, max, type, message) {
	...
	if (_env.iOS) win_assetGroups.setLeftNavButton(cancel);else win_assetGroups.add(cancel);
	...
	var gotAssets = function (e) {
		...
		win_assets.addEventListener('close', function () {
			...
			for (var i = 0; i < assets.length; i++) {
				for (var j = 0; j < assets[i].length; j++) {
					...
					if (!iv) continue;
					iv.image = null;
					if (_env.iOS) rows[iv.row].remove(iv);else grid.remove(iv);
					iv = null;
				}
			}
		...
		if (_env.iOS) win_assets.setRightNavButton(done);else win_assets.add(done);
	...
	function createIV(row, i) {
		...
		iv.addEventListener('longpress', function (e) {
			...
			if (type == 'videos') {
				...
				var iv = e.source;
				if (_env.iOS) activeMovie.media = assets[iv.row][iv.i].url;else activeMovie.url = 'file://' + assets[iv.row][iv.i].url;
