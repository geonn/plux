!function() {
    ...
    function Axis(chart, options, type, position) {
        ...
        } else "left" === position || "top" === position ? this.sessionVariables = this.chart.sessionVariables["axisY"] : this.sessionVariables = this.chart.sessionVariables["axisY2"];
    ...
    CanvasJSObject.prototype.setOptions = function(options, currentThemeOptions) {
        if (defaultOptions[this._defaultsKey]) {
            ...
            for (var prop in defaults) options && prop in options ? this[prop] = options[prop] : currentThemeOptions && prop in currentThemeOptions ? this[prop] = currentThemeOptions[prop] : this[prop] = defaults[prop];
    ...
    CanvasJSObject.prototype.isBeingTracked = function(option) {
        this._options._oldOptions || (this._options._oldOptions = {});
        return !!this._options._oldOptions[option];
    };
        ...
        if (this._options.zoomEnabled) {
            ...
            if (!this._resetButton) {
                ...
                addEvent(this._resetButton, "click", function() {
                    ...
                    _this._options.axisX && _this._options.axisX.minimum ? _this.sessionVariables.axisX.internalMinimum = _this._options.axisX.minimum : _this.sessionVariables.axisX.internalMinimum = null;
                    _this._options.axisX && _this._options.axisX.maximum ? _this.sessionVariables.axisX.internalMaximum = _this._options.axisX.maximum : _this.sessionVariables.axisX.internalMaximum = null;
    ...
    Chart.prototype._initialize = function() {
        ...
        for (var series = 0; series < this._options.data.length; series++) {
            ...
            } else "line" === dataSeries.type || "stepLine" === dataSeries.type || "spline" === dataSeries.type || "area" === dataSeries.type || "stepArea" === dataSeries.type || "splineArea" === dataSeries.type || "stackedArea" === dataSeries.type || "stackedArea100" === dataSeries.type || "rangeArea" === dataSeries.type || "rangeSplineArea" === dataSeries.type || "candlestick" === dataSeries.type || "ohlc" === dataSeries.type ? dataSeries._colorSet = [ this._selectedColorSet[0] ] : dataSeries._colorSet = this._selectedColorSet; else dataSeries._colorSet = [ dataSeries.color ];
    ...
    Chart.prototype._mouseEventHandler = function(ev) {
        ...
        if (Chart.capturedEventParam) {
            ...
            eventParam && eventParam.cursor ? ev.target.style.cursor = eventParam.cursor : ev.target.style.cursor = this._defaultCursor;
        }
    ...
    Chart.prototype._plotAreaMouseDown = function(x, y) {
        this.isDrag = true;
        "none" !== this.plotInfo.axisPlacement ? this.dragStartPoint = {
            x: x,
            y: y,
    ...
    Chart.prototype.renderColumn = function(plotUnit) {
        ...
        barWidth > maxBarWidth ? barWidth = maxBarWidth : xMinDiff === 1 / 0 ? barWidth = maxBarWidth : 1 > barWidth && (barWidth = 1);
        ctx.save();
        ...
        for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
            ...
            var dataPoints = dataSeries.dataPoints;
            if (dataPoints.length > 0) {
                var bevelEnabled = !!(barWidth > 5 && dataSeries.bevelEnabled);
                for (i = 0; i < dataPoints.length; i++) {
                    dataPointX = dataPoints[i].getTime ? dataPoints[i].x.getTime() : dataPoints[i].x;
                    if (dataPointX < plotUnit.axisX.dataInfo.viewPortMin || dataPointX > plotUnit.axisX.dataInfo.viewPortMax) continue;
    ...
    Chart.prototype.renderPie = function(plotUnit) {
        function initLabels() {
            ...
            for (j = 0; j < dataPoints.length; j++) {
                ...
                midAngle > Math.PI / 2 && midAngle <= 3 * Math.PI / 2 ? dataPointEO.hemisphere = "left" : dataPointEO.hemisphere = "right";
        ...
        this.pieDoughnutClickHandler = function(e) {
            ...
            dataPoint.exploded ? dataPoint.exploded = false : dataPoint.exploded = true;
    ...
    Axis.prototype.renderLabelsTicksAndTitle = function() {
        ...
        if ("bottom" === this._position) {
            ...
            for (var i = 0; i < this._labels.length; i++) {
                ...
                0 === this.labelAngle ? label.textBlock.y = xy.y - label.textBlock.height / 2 + this.labelFontSize / 2 : label.textBlock.y = xy.y - label.textBlock.width * Math.sin(Math.PI / 180 * this.labelAngle);
                label.textBlock.render(true);
            }
            if (this.title) {
                this._titleTextBlock = new TextBlock(this.ctx, {
            ...
            for (var i = 0; i < this._labels.length; i++) {
                ...
                0 === this.labelAngle ? label.textBlock.y = xy.y - label.textBlock.height / 2 + this.labelFontSize / 2 : label.textBlock.y = xy.y;
                label.textBlock.render(true);
            }
            if (this.title) {
                this._titleTextBlock = new TextBlock(this.ctx, {
        ...
        if ("bottom" !== this._position && "top" !== this._position || !this.interlacedColor) {
            if (("left" === this._position || "right" === this._position) && this.interlacedColor) {
                ...
                for (i = 0; i < this._labels.length; i++) {
                    ...
                    if (renderInterlacedGrid) {
                        ...
                        interlacedGridStartPoint = i + 1 >= this._labels.length ? this.getPixelCoordinatesOnAxis(this.maximum) : this.getPixelCoordinatesOnAxis(this._labels[i + 1].position);
            ...
            for (i = 0; i < this._labels.length; i++) {
                ...
                if (renderInterlacedGrid) {
                    ...
                    interlacedGridEndPoint = i + 1 >= this._labels.length ? this.getPixelCoordinatesOnAxis(this.maximum) : this.getPixelCoordinatesOnAxis(this._labels[i + 1].position);
    ...
    Axis.prototype.calculateAxisParameters = function() {
        ...
        if ("axisX" === this.type) {
            ...
            minDiff = this.dataInfo.minDiff !== 1 / 0 ? this.dataInfo.minDiff : max - min > 1 ? .5 * Math.abs(max - min) : 1;
        ...
        if ("axisX" === this.type && "dateTime" === this.chart.plotInfo.axisXValueType) {
            ...
            null !== this.sessionVariables.internalMinimum ? this.minimum = this.sessionVariables.internalMinimum : this.minimum = min - minDiff / 2;
            this.sessionVariables.internalMaximum ? this.maximum = this.sessionVariables.internalMaximum : this.maximum = max + minDiff / 2;
            ...
            this._options && this._options.interval ? this.interval = this._options.interval : this.interval = Axis.getNiceNumber(range / (noTicks - 1), true);
            null !== this.sessionVariables.internalMinimum ? this.minimum = this.sessionVariables.internalMinimum : this.minimum = Math.floor(min / this.interval) * this.interval;
            null !== this.sessionVariables.internalMaximum ? this.maximum = this.sessionVariables.internalMaximum : this.maximum = Math.ceil(max / this.interval) * this.interval;
            if ("axisX" === this.type) {
