...
exports.M = function(name, modelDesc, migrations) {
    ...
    } else extendObj.sync = function(method, model, opts) {
...
exports.createStyle = function(controller, opts, defaults) {
    ...
    for (i = 0, len = styleArray.length; len > i; i++) {
        ...
        if (style.queries && style.queries.formFactor && !exports[style.queries.formFactor]) continue;
        if (style.queries && style.queries["if"] && ("false" === style.queries["if"].trim().toLowerCase() || -1 !== style.queries["if"].indexOf("Alloy.Globals") && false === exports.Globals[style.queries["if"].split(".")[2]])) continue;
