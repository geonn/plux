...
function Controller() {
  ...
  { backgroundColor: "#535a74", orientationModes: [Ti.UI.PORTRAIT], fullscreen: false, windowSoftInputMode: Ti.UI.Android.SOFT_INPUT_STATE_HIDDEN, id: "win", title: "ASP eCARD", backButtonTitle: "", navTintColor: "#CE1D1C" });

  $.__views.win && $.addTopLevelView($.__views.win);
  $.__views.main = Ti.UI.createView(
  { borderWidth: 0, layout: "vertical", width: Ti.UI.FILL, height: Ti.UI.FILL, id: "main" });

  $.__views.win.add($.__views.main);
  if (true) {
    $.__views.__alloyId384 = Ti.UI.createView(
    { borderWidth: 0, top: 0, layout: "horizontal", height: 50, width: Ti.UI.FILL, backgroundColor: "#DEDEDE", id: "__alloyId384" });

    $.__views.main.add($.__views.__alloyId384);
    $.__views.__alloyId385 = Ti.UI.createView(
    { borderWidth: 0, left: 0, width: "20%", id: "__alloyId385" });

    $.__views.__alloyId384.add($.__views.__alloyId385);
    $.__views.btnBack = Ti.UI.createImageView(
    { left: 10, id: "btnBack", width: 25, height: 25, image: "/images/btn-back.png" });

    $.__views.__alloyId385.add($.__views.btnBack);
    $.__views.__alloyId386 = Ti.UI.createView(
    { borderWidth: 0, width: "60%", id: "__alloyId386" });

    $.__views.__alloyId384.add($.__views.__alloyId386);
    $.__views.pageTitle = Ti.UI.createLabel(
    { width: Titanium.UI.SIZE, height: Ti.UI.SIZE, color: "#111111", font: { fontSize: "16dp" }, text: 'eCard List', id: "pageTitle", textAlign: "center" });

    $.__views.__alloyId386.add($.__views.pageTitle);
  }
  ...
  function render_ecard_list(data) {
    var card_width = (false ? Ti.Platform.displayCaps.platformWidth : parseInt(Ti.Platform.displayCaps.platformWidth / (Ti.Platform.displayCaps.logicalDensityFactor || 1), 10)) - 20;
    var card_height = card_width / 2;
    var inner_width = card_width * 1.5;
    var inner_height = card_width * 1.5;
    var inner_top = -((inner_height - card_height) / 2) * 1.333;
    var inner_left = -((inner_width - card_width) / 2) * 1.6;

    console.log(card_width + " " + card_height + " card size");
  ...
  var guide_message = ["Welcome to ASP HEALTHCARE, I'm ASP assistant to guide you the ECard feature", "Can you see the QR Code? but you cant scan it here. You need to click on it to make it bigger.", "Now you can see the the QR Code change every second. It will prevent the bad guy screenshot your qr and use your ECard.", "Here is your personal info summary. Click inside for more detail.", "You can see your expired date here, so dont forget to renew.", "Last one is your name " + Ti.App.Properties.getString('fullname'), "That's all for the guideline. Good Bye."];
  function toolTipGuide() {
    var child_num = false ? 0 : 1;
  ...
  function zoomIn(e) {
    ...
    if (false) {
      ...
      e.parent.animate({ transform: Ti.UI.create2DMatrix({ rotate: 0 }), duration: 1000 });
      e.animate({ transform: Ti.UI.create2DMatrix({ rotate: 0 }), duration: 500 }, function () {
  ...
  function zoomQr(e) {
    ...
    if (!interval) {
      interval = setInterval(function () {
        render_qr(e.source, e.source.rect.width);
      }, 1000);
    } else {
