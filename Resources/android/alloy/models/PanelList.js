    ...
    extendCollection: function(Collection) {
        _.extend(Collection.prototype, {
            ...
            getCountClinicType: function(corp) {
                var collection = this;
                if ("" != corp) var sql = "SELECT clinicType, count(DISTINCT(id)) as total FROM " + collection.config.adapter.collection_name + " where panel=1 AND status = 1 GROUP BY clinicType "; else var sql = "SELECT clinicType, count(DISTINCT(id)) as total FROM " + collection.config.adapter.collection_name + " WHERE status = 1 GROUP BY clinicType ";
                db = Ti.Database.open(collection.config.adapter.db_name);
                var res = db.execute(sql);
                var listArr = [];
                var count = 0;
