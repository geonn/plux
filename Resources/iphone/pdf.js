...
function pdf(url, cookies, inds, labels, indView, done) {
    ...
    download(url, cookies, function(err, file, base, url) {
        ...
        if (false === tempFile) {
            ...
            "android" == Ti.Platform.osname ? done(err, tempFile, base, url) : done(err, file, base, url);
        } else "android" == Ti.Platform.osname ? done(err, tempFile, base, url) : done(err, file, base, url);
    });
}
